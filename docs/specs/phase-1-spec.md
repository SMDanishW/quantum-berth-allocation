...
5. Due dates — **AMENDED 2026-07-17 (spike B):** `target_departure = eta + d_v^min` (EFT, soft due date); `latest_departure = eta + ceil(1.5 × d_v^min)` (LFT, hard deadline). Aligns the generator with the amended T1.2 M&B semantics (§4.2), so tardiness means the same thing across both instance families and synthetic instances regain hard-deadline domain pruning. The slack factor stays **1.5**: it is proportional to each vessel's own `d_v^min`, so the lognormal (σ=0.83) service-time spread scales slack per vessel exactly as in M&B's uniform world — the calibration data gives no reason for a synthetic-specific factor. **V6 always holds** under the existing horizon formula: `latest = eta + ceil(1.5·d) ≤ max(eta) + 2·d ≤ max(eta) + 3·max(d) = time_horizon`, using `ceil(1.5d) ≤ 2d` for integer `d ≥ 1`. Side effect (accepted, same as M&B): a hard LFT means sufficiently congested draws can be genuinely infeasible — that is a property of the instance, not a generator bug.
   *Superseded text:* `target_departure = eta + ceil(1.5 × d_v^min)`; `latest_departure = None`.
...
Required tests: same seed ⇒ identical `model_dump_json`; different seed ⇒ different; every instance over a seed sweep passes validation; congestion monotonicity per D5 AC; horizon-cap raises. **AMENDED 2026-07-17 (spike B):** additionally, on a fixed seed assert `target_departure == eta + d_min` and `latest_departure == eta + ceil(1.5 × d_min)` per vessel; over the seed sweep assert `latest_departure ≤ time_horizon` (V6 argument in step 5).
